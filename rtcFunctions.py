...
class RTCInitializer:
    ...
    def initialize():
        ...
        if config.useexistingworkspace:
            ...
            workspace.createandload(
                config.streamuuid, config.initialcomponentbaselines)
...
class RTCLogin:
    ...
    def loginandcollectstreamuuid():
        ...
        if not config.stored:
            ...
            exitcode = shell.execute((loginHeaderCommand + loginCredentialsCommand) % (
                config.scmcommand, config.repo, config.user, config.password))
...
class WorkspaceHandler:
    ...
    def createandload(self, stream, componentbaselineentries=[]):
        shell.execute("%s create workspace -r %s -s %s %s" %
                      (self.scmcommand, self.repo, stream, self.workspace))
    ...
    def load(self):
        command = "%s load -r %s %s --force" % (
            self.scmcommand, self.repo, self.workspace)
    ...
    def setcomponentstobaseline(self, componentbaselineentries, streamuuid):
        for entry in componentbaselineentries:
            ...
            replacecommand = "%s set component -r %s -b %s %s stream %s %s --overwrite-uncommitted" % \
                             (self.scmcommand, self.repo, entry.baseline,
                              self.workspace, streamuuid, entry.component)
    ...
    def setnewflowtargets(self, streamuuid):
        ...
        if not self.hasflowtarget(streamuuid):
            shell.execute("%s add flowtarget -r %s %s %s" %
                          (self.scmcommand, self.repo, self.workspace, streamuuid))
    ...
    def hasflowtarget(self, streamuuid):
        command = "%s --show-uuid y --show-alias n list flowtargets -r %s %s" % (
            self.scmcommand, self.repo, self.workspace)
...
class Changes:
    ...
    def discard(*changeentries):
        ...
        exitcode = shell.execute(config.scmcommand + " discard -w " +
                                 config.workspace + " -r " + config.repo + " -o" + idstodiscard)
        if exitcode is 0:
...
class ImportHandler:
    ...
    def getcomponentbaselineentriesfromstream(self, stream):
        filename = self.config.getlogpath(
            "StreamComponents_" + stream + ".txt")
    ...
    def determineinitialbaseline(self, stream):
        ...
        logincredentials = ""
        if not config.stored:
            logincredentials = loginCredentialsCommand % (
    ...
    def acceptchangesintoworkspace(self, changeentries):
        ...
        if amountofchanges == 0:
            ...
            shouter.shoutwithdate(
                "Start accepting %s changesets" % amountofchanges)
        ...
        for changeEntry in changeentries:
            ...
            if not changeEntry.isAccepted():  # change could already be accepted from a retry
                ...
                if not Differ.has_diff():
                    ...
                    shouter.shout(
                        "No changes for commiting in git detected, going to reload the workspace")
                    WorkspaceHandler().load()
                    if not Differ.has_diff():
                        shouter.shout(
                            "Still no changes... Please load your workspace in eclipse and check whats wrong")
    ...
    def collect_changes_to_accept_to_avoid_conflicts(changewhichcantbeacceptedalone, changes, maxchangesetstoaccepttogether):
        ...
        nextchange = ImportHandler.getnextchangeset_fromsamecomponent(
            changewhichcantbeacceptedalone, changes)
    ...
    def retryacceptincludingnextchangesets(self, change, changes):
        issuccessful = False
        changestoaccept = ImportHandler.collect_changes_to_accept_to_avoid_conflicts(
            change, changes, self.config.maxchangesetstoaccepttogether)
        ...
        if amountofchangestoaccept > 1:
            ...
            if self.config.useautomaticconflictresolution or self.is_user_agreeing_to_accept_next_change(change):
                shouter.shout(
                    "Trying to resolve conflict by accepting multiple changes")
                for index in range(1, amountofchangestoaccept):
                    # accept least possible amount of changes
                    toaccept = changestoaccept[0:index + 1]
    ...
    def getchangeentriesofstreamcomponents(self, componentbaselineentries):
        ...
        for componentBaseLineEntry in componentbaselineentries:
            ...
            changeentries = self.getchangeentriesofbaseline(
                componentBaseLineEntry.baseline)
            for changeentry in changeentries:
    ...
    def readhistory(self, componentbaselineentries, streamname):
        ...
        for componentBaseLineEntry in componentbaselineentries:
            history = self.gethistory(
                componentBaseLineEntry.componentname, streamname)
    ...
    def getchangeentriestoaccept(missingchangeentries, history):
        ...
        if history:
            ...
            changeentriestoaccept = sorter.tosortedlist(
                historywithchangeentryobject)
        else:
            ...
            for line in file:
                ...
                if cleanedline:
                    if cleanedline.startswith(componentprefix):
                        ...
                        component = cleanedline[length:cleanedline.index(
                            ")", length)]
                    else:
                        currentinformationpresent += cleanedline.count(
                            informationseparator)
                        if currentline:
                        ...
                        if currentinformationpresent >= numberofexpectedinformationseparators:
                            ...
                            changeentries.append(ChangeEntry(
                                revision, author, email, date, comment, component))
    ...
    def getchangeentriesofstream(self, streamtocompare):
        ...
        missingchangeentries = {}
        changeentries = self.getchangeentriesbytypeandvalue(
            CompareType.stream, streamtocompare)
        for changeentry in changeentries:
            missingchangeentries[changeentry.revision] = changeentry
        return missingchangeentries
    ...
    def getchangeentriesofworkspace(self, workspacetocompare):
        missingchangeentries = {}
        changeentries = self.getchangeentriesbytypeandvalue(
            CompareType.workspace, workspacetocompare)
        for changeentry in changeentries:
            missingchangeentries[changeentry.revision] = changeentry
        return missingchangeentries
    ...
    def getchangeentriesbytypeandvalue(self, comparetype, value):
        ...
        outputfilename = self.config.getlogpath(
            "Compare_" + comparetype.name + "_" + value + ".txt")
    ...
    def gethistory(self, componentname, streamname):
        outputfilename = self.config.gethistorypath(
            "History_%s_%s.txt" % (componentname, streamname))
...
class ChangeEntry:
    ...
    def tostring(self):
        return "%s (Date: %s, Author: %s, Revision: %s, Component: %s, Accepted: %s)" % \
               (self.comment, self.date, self.author,
                self.revision, self.component, self.accepted)
